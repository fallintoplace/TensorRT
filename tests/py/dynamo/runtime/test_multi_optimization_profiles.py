...
class TestMultiProfileRuntime(TestCase):
    ...
    def _check_reconstructed_profile_state(self, engine):
        ...
        # internal ``_profile_dim_ranges`` / ``_auto_select_profile`` (not part
        # of the user-facing API; the C++ runtime is covered behaviorally below).
        self.assertEqual(engine.num_optimization_profiles, 2)

        ranges = engine._profile_dim_ranges
        name = next(iter(ranges))
        # Ranges for the dynamic seq axis (dim 1). tuple() normalizes the Python
        # runtime (tuples) and C++ runtime (lists) to compare uniformly.
        seq_ranges = ranges[name][1]
